...
class BoardMapper:
    ...
    def create(self, fen: str) -> NDArray:
        """
...
class PolicyMapper:
    """
    Mapping class to convert policy dictionaries into numpy arrays.
    """

    def __init__(self) -> None:
        """
        Initialize mapper, load keys.

        """
        self.move_list = read_file("data/moves.txt")
        self.move_length = len(self.move_list)

    def create(
        self,
        policy_dict: list[dict[str, float]],
    ) -> tuple[NDArray, NDArray]:
        """
        Turn policy dictionary into a numpy array.

        Parameters
        ----------
        policy_array : list[dict[str, float]]
            Policy dictionary of form {move UCI : value}.
        Returns
        -------
        Tuple[NDArray, NDArray]
            A tuple where the first item is the numpy array and the second item is the
            mask.
        """
        policy_array = np.zeros(self.move_length)

        # Iterate over all possible keys
        for i, key in enumerate(self.move_list):
            # If the key exists in the dictionary, fill in the corresponding value
            if key in policy_dict:
                policy_array[i] = policy_dict[key]

        # Create a mask where values in the array are greater than zero
        mask_array = policy_array > 0

        # if there are no legal moves, fill array so that probabilities still sum to 1
        if not mask_array.any():
            policy_array = np.full(self.move_length, 1 / self.move_length)
        return policy_array, mask_array
