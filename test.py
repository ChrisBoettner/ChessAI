...
class Game(object):
    def __init__(self, engine:str='stockfish',
                       ...
                       engine_skill_level:int = 10) -> None:
    ...
    def continue_game(self) -> None:
        raise NotImplementedError('Does not quite work yet, since we do not '
    ...
    def _game(self) -> None:
        # player order
        turns = ([self._player_turn, self._engine_turn]
    ...
    def _engine_turn(self) -> chess.Move:
        engine_move = self.engine.play(self.board, 
                            chess.engine.Limit(time=self.engine_limit)).move
        return(engine_move)
